...
def react_router_node(state: AgentState):
    ...
    try:
        ...
        print("调用 Agent (异步)...")
        try:
            response = asyncio.run(agent.ainvoke(agent_input))
            print("异步调用成功")
            ...
            if "messages" in response:
                ...
                for i, msg in enumerate(response['messages']):
                    if hasattr(msg, 'content'):
                        ...
                        if isinstance(content, str) and 'access_token' in content.lower() and state.get("client_token") is None:
